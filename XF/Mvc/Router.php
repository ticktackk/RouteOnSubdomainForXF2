...
class Router extends XFCP_Router
{
    ...
    /**
     * Sets $subDomainSupportEnabled and $primaryHost values when route filter is set which is only allowed for pub
     * router.
     *
     * @param string $name
    ...
    public function addRoutePreProcessor($name, $preProcessor, $beginning = false)
    {
        ...
        parent::addRoutePreProcessor($name, $preProcessor, $beginning); // TODO: Change the autogenerated stub
    }
    ...
    protected function tckRouteOnSubdomainGenerateRedirectLink(string $route, string $path, Request $request = null) : string
    {
        ...
        $redirectUrl = "{$protocol}://{$route}.{$this->primaryHost}" . ($useFriendlyUrls ? '/' : '/index.php');
    ...
    public function buildFinalUrl($modifier, $routeUrl, array $parameters = [])
    {
        ...
        if ($this->subDomainSupportEnabled && $modifier === 'canonical')
        {
            $modifier = null;
        }
